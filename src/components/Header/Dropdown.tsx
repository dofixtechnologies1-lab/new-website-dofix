...
const Dropdown = ({ menuItem, stickyMenu }) => {
  ...
  return (
    ...
    >
      <a
        href="#"
        className={`hover:text-[#3683ab] text-custom-sm font-medium text-dark flex items-center gap-1.5 capitalize ${
          stickyMenu ? "xl:py-4" : "xl:py-6"
        } ${pathUrl.includes(menuItem.title) && "!text-[#3683ab]"}`}
      >
        {menuItem.title}
        <svg
      ...
      >
        {menuItem.submenu.map((item, i) => (
          <li key={i}>
            <Link
              href={item.path}
              className={`flex text-custom-sm hover:text-[#3683ab] hover:bg-gray-1 py-[7px] px-4.5 ${
                pathUrl === item.path && "text-[#3683ab] bg-gray-1"
              } `}
            >
              {item.title}
            </Link>
